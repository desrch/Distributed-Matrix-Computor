...
class HybridSpMVEngine : public ISpMVEngine {
public:
    std::string name() const override {
        auto& ctx = DistributedContext::instance();
        std::string s = "Hybrid_p" + std::to_string(ctx.size()) + "_t";
#if defined(_OPENMP)
        s += std::to_string(omp_get_max_threads());
#else
        s += "1";
#endif
        return s;
