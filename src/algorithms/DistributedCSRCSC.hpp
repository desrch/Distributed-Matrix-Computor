...
/**
 * @brief 分布式 CSR × CSC → CSR
 *
 * 策略: root 上将 B(CSC) 转为 CSR，广播，各 rank 执行局部 SpGEMM。
 *
 * 流水线:
 *   Master 持有完整 CSR A(M×K) 和 CSC B(K×N)
 *        │
 *        ├─ 1. Root: B_csc → B_csr (toCSR)
 *        ├─ 2. 广播 M,K,N + row_ptr_A, 按行 Scatter CSR A
 *        ├─ 3. Broadcast  完整 CSR B
 *        ├─ 4. 各 rank 局部 SpGEMM: local_C = local_A × B_csr
 *        └─ 5. Gather     CSR 三数组 → Master 拼出完整 C
 *
 * Gather 策略:
 ...
 */
class DistributedCSRCSC {
public:
    static CSRMatrix multiply(const CSRMatrix& A, const CSCMatrix& B)
    {
        ...
        // 4b. per-row nnz → gather → 重建 row_ptr
